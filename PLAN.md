...
- 🔒 **HARD — Reuse before you write; no overlapping helpers.** Before writing any helper/util, `grep` for an existing one that already does it (search `lib/` first). Extend or reuse it rather than adding a near-duplicate. Do not create several functions that do almost the same thing (e.g. three slightly different `getStation*` or date-range helpers). One canonical helper per concept, placed in `lib/`. If two call sites need the same logic, factor it into one shared function — do not copy-paste it.
- 🔒 **HARD — No type escape hatches.** Do not use `any`, `as any`, `as unknown as X`, `@ts-ignore`, `@ts-expect-error`, or non-null assertions (`!`) to silence the type checker in **new** code. A type error is signal that something is actually wrong — fix the cause (correct the type, narrow properly, update the interface in `lib/types.ts`). Add/extend real interfaces for the new `station_id` fields and tenant tables rather than casting around them. (Exception: the one pre-existing `(_supabaseAdmin as any)[prop]` Proxy cast in `lib/supabase.ts` may stay as-is — do not propagate that pattern to new code.) If a type genuinely cannot be expressed, stop and ask rather than casting.
- 🔒 **HARD — Modular files AND modular functions; no monoliths, no god-functions.** Follow the existing module layout: shared logic in `lib/`, one concern per file, route logic in its `route.ts`, worker logic in its worker file. Do not pile unrelated functionality into one large file, and do not grow one giant function by appending "just one more case" to the bottom. Each function does one thing; prefer early-return guard clauses over deep nesting; extract a sub-step into a named helper rather than nesting another level. 🔧 FLEXIBLE — rough guides (not hard limits): a new file much past ~300 lines, a single function much past ~60 lines, or nesting deeper than ~3–4 levels is a smell to split, not an automatic violation. The real rule: when you catch yourself adding to a function/file that's already large, stop and factor it out instead. Match the granularity of the surrounding code.
- 🔒 **HARD — No silent fallbacks, no swallowed errors.** Failures and fallbacks must be visible, never hidden. Specifically: (a) no empty `catch {}` or `catch (e) {}` that drops the error — handle it, or log it and rethrow/surface it; (b) the per-station → global settings fallback (Phase G) and the `mp3_filename_prefix` default (Phase F) are the *only* sanctioned fallbacks, and each must be intentional and logged when it triggers, not a silent swallow; (c) never auto-pick or default a `station_id`/active station when context is missing — return a clear 401/403/400 instead (silently defaulting a station is a cross-tenant data risk); (d) do not catch an error just to return empty data that looks like a valid "no results." A caller (and the user) must be able to tell "this station has no episodes" apart from "the query failed." If you add a fallback, say so in the code and in your commit message.
...
8. No new `any`/`@ts-ignore`/type escape hatches introduced (re-grep the diff for `as any`, `@ts-ignore`, `@ts-expect-error`); no duplicate/overlapping helpers; no new god-files.
