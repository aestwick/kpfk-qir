...
- 🔒 **HARD — Modular files, not monoliths.** Follow the existing module layout: shared logic in `lib/`, one concern per file, route logic in its `route.ts`, worker logic in its worker file. Do not pile unrelated functionality into one large file. 🔧 FLEXIBLE — as a rough guide, if a new file pushes much past ~300 lines or mixes clearly separate concerns, split it into focused modules; size alone isn't a violation, but a god-file is. Match the granularity of the surrounding code.
...
3. 🔒 **HARD — Run an adversarial review in a fresh subagent**: "Review the full diff against PLAN.md. Verify every phase's requirements are implemented, every API route in the Phase E checklist is station-filtered, every worker query (including claim guards) is station-scoped, RLS isolation actually holds, and nothing outside this plan's scope was changed. Also flag: any new `any`/`as any`/`@ts-ignore`/`@ts-expect-error`/type escape hatches, duplicate or overlapping helpers that should have been a single shared function, and oversized god-files that should have been split. Report only correctness/requirement gaps, the listed code-quality violations, and out-of-scope changes — not style preferences." Fix real gaps it finds; do not chase style nits into over-engineering.
