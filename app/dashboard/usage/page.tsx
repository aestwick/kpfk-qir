      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <div className="bg-white rounded-lg shadow p-4">
              <p className="text-xs text-gray-500">Groq (Transcription)</p>
              <p className="text-xl font-bold">{formatCost(totals?.groq ?? 0)}</p>
            </div>
            <div className="bg-white rounded-lg shadow p-4">
              <p className="text-xs text-gray-500">OpenAI (Summarization)</p>
              <p className="text-xl font-bold">{formatCost(totals?.openai ?? 0)}</p>
            </div>
            <div className="bg-white rounded-lg shadow p-4">
              <p className="text-xs text-gray-500">Episodes Processed</p>
          ...
          {totals?.byOperation && Object.keys(totals.byOperation).length > 0 && (
            <div className="bg-white rounded-lg shadow p-4">
              <h3 className="font-semibold text-sm text-gray-500 uppercase mb-3">Cost by Operation</h3>
          ...
          <div className="bg-white rounded-lg shadow overflow-hidden">
            <div className="px-4 py-3 border-b">
              <h3 className="font-semibold text-sm text-gray-500 uppercase">Daily Spend</h3>
            </div>
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y">
                {sortedDays.length === 0 ? (
                  <tr><td colSpan={5} className="px-4 py-6 text-center text-gray-500">No usage data for this period</td></tr>
                ) : sortedDays.map((day) => (
                  <tr key={day}>
