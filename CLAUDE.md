...
lib/
  ...
  aircheck.ts       — Aircheck engine (pure): merges a night's captions onto the
                      wall clock per stream, scores the :00/:30 station-ID marks,
                      detects promos (EN/ES) + start/stop hygiene
                      (driver: scripts/scan-airchecks.ts)
  aircheck-report.ts — Aircheck markdown rendering + per-show scorecard (pure)
...
**Aircheck scanning** (`npm run scan-airchecks -- --station <slug>`) answers "are hosts identifying the station on the hour, running promos, and starting/stopping cleanly?" for a daypart (default the last 14 station-local days, 20:00–24:00; `--start`/`--end` move the daypart, `--from`/`--to` the dates). The archive slices recordings on the hour and half-hour, so **a file's own head IS a boundary** — which is why the engine maps every caption cue to absolute wall-clock time and merges the night into one timeline **per stream** before scoring: an ID given by the outgoing host at 20:29:40 satisfies the 20:30 mark even though it lands in the previous file, and scoring files in isolation would report that as missing. Streams are kept apart because KPFK's second channel (`2kpfk_…` filenames) overlaps the main signal in wall-clock time and only the main signal carries the FCC legal-ID obligation (`streamOf`). IDs are graded **legal** / **callsign** / **frequency** / **none**, built from `stations.station_id_patterns` plus the Spanish spoken frequency — the evening block is majority Spanish-language and the configured patterns are English-only. **`legal` follows 47 CFR 73.1201(b)(1)**: the call letters must be *immediately followed by* the community of license, with only the licensee name / frequency / channel permitted in between. That gap is checked by **vocabulary, not character distance** (`LEGAL_ID_FILLER`) — the spacing in a real spoken-frequency ID ("KPFK ninety point seven FM Los Angeles") is nearly identical to that in a non-ID ("KPFK. Our guest joins us from Los Angeles"), so a distance threshold cannot separate them. A boundary also records `cityInWindow`, so a report can distinguish "never said the city" from "said it, but not in legal-ID form". A mark with **no captions in the window is "not scorable", never a miss** (`isMiss` in `lib/aircheck-report.ts`): a failed transcription or a music hour with nothing to transcribe must not be reported as a host failing to ID. Start/stop hygiene flags `head_dead_air`, `cold_open` (opens mid-sentence), `prior_show_bleed` (the *previous* programme's sign-off lands inside this file), `tail_dead_air`, `hard_cut` and `no_signoff`; the two tail checks are skipped when `duration` is null, since the slot length then comes from where the captions stop and the tail gap would be zero by construction. Promo detection is a bilingual lexical **floor** (tune-in / membership / underwriting / stream plug); `--verify` adds a gpt-4o-mini pass over the files with no lexical hit (logged to `usage_log` as operation `verify`, honors the spend limits). Output: terminal summary + markdown/JSON in `reports/` (gitignored); each run leaves an `audit_log` row (`aircheck.scan.complete`). A review aid, not pass/fail — machine transcription is imperfect, so a human confirms against the audio before anyone is told off. Engine + renderer are pure and unit-tested (`lib/aircheck.ts`, `lib/aircheck-report.ts`).
