...
**FCC content-category taxonomy (migration 033).** The three federal categories are modeled as distinct flag types, mapped to their actual legal treatment: **obscenity** (patently offensive *descriptions of sexual acts*, Miller standard) is **not protected speech and has no safe harbor** — AI-detected, always `critical`, raised 24/7; **indecency** (the Carlin "seven dirty words", FCC v. Pacifica) and **profanity** (the amorphous catch-all *beyond* the Carlin list) are **protected but safe-harbor-restricted** — both wordlist-driven and downgraded to `warning` outside the 6am–10pm restricted window. The `compliance_wordlist` carries a `category` column (`'indecency' | 'profanity'`, default `'profanity'`) that becomes the emitted `flag_type`; the Carlin words are seeded as `'indecency'`, everything else (slurs, local additions) is `'profanity'`. The AI `compliance_prompt` emits `obscenity` only. Toggles live in `compliance_checks_enabled` (`obscenity`, `indecency`, `profanity`, …).
