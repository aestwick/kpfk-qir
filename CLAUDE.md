...
app/api/
  ...
  v1/                       — Public API (API-key auth, GET-only except /transcribe). See "Public Read API".
    transcribe/route.ts           — POST on-demand transcription by mp3_url (find-or-create
                                    episode → targeted transcribe job → writes transcripts table,
                                    no summarize chain); GET ?mp3_url= status probe. WRITE scope `transcribe`.
