...
lib/
  ...
  verify-week.ts    — Broadcast-week verification engine (pure): expands the CMS
                      schedule into expected blocks, matches airings by show
                      identity + time overlap (driver: scripts/verify-week.ts)
  pitch-report.ts   — Fund-drive pitch analytics engine (pure): tiered lexicon →
                      scored cues → merged pitch segments → per-show/hour/day
                      roll-ups + markdown/CSV (driver: scripts/pitch-report.ts)
...
**Steady-state is current-quarter-scoped; backfills opt out via a window.** The transcribe + summarize candidate queries are pinned to the **current calendar quarter** (`getCurrentQuarterBounds()`) — that's the live FCC cadence and what the dashboard backlog reflects. A historical one-off (e.g. retro-generating WPFW's Q4 2020 QIR from an archive backup) gets past that gate by passing an explicit `{ window: { start, end } }` in the transcribe/summarize job data; `resolveWindow(job)` uses it instead of the current quarter, and the continue/backoff/chain re-enqueues (`workers/index.ts`) thread the same window through the whole drain. QIR *generation* was never quarter-locked (`generate-qir.ts` is parameterized by `year`/`quarter`). A windowed backfill also **bypasses the per-station pause** across **all three expensive stages** (transcribe/summarize/**compliance** processors + the transcribe→summarize→compliance chain treat a `window`-bearing job as an explicit operator action) so a *parked* station can still drain a one-off backfill end-to-end — the **global** kill switch still stops it. `scripts/backfill-quarter.ts --kick` re-enqueues the windowed chain **and** a windowed compliance sweep (which checks episodes already `summarized` before the chain reached compliance). Driver: **`scripts/backfill-quarter.ts`** (`npm run backfill-quarter -- --station <slug> --year <Y> --quarter <Q> --urls <file>`) inserts a flat list of archive MP3 URLs as `pending` episodes (dates/show keys parsed from the filename, show names + categories resolved from each show's live feed, duration filled from the audio at transcription), kicks the windowed chain, and — with `--generate` — enqueues the draft once they've summarized. The Generate page's quarter picker reaches back 6 years so a backfilled quarter is reviewable/finalizable in the UI.

**Broadcast-week verification** (`npm run verify-week -- --station <slug>`) answers "did what aired match what we expected to air?" for a date window (default: the last 7 station-local days). The *expected* side comes read-only from the sibling CMS app's tables in the shared database — `cms_schedule_published` (concrete per-date rows) when present for a date, else the recurring `cms_schedule_slots` weekly grid — matched to QIR airings (`episode_log`) by show identity + time overlap. Identity resolves as: the slot's CMS show → its `cms_show_source.qir_show_key` mappings + `program_slug`, **expanded through QIR's own `show_keys.show_group`** (one logical show spans multiple feed keys — e.g. the overnight strip logs six hourly keys), with a normalized-name fallback for label-only slots. Blocks whose show has no *active* `show_keys` row report as "not tracked" (QIR deliberately skips music/arts shows), not "missing". Layer 1 (always, free) reports per day: aired / partial / missing blocks, unscheduled airings (and what they displaced), plus pipeline issues — including `pending` episodes stranded behind the current-quarter gate (fix: `backfill-quarter --kick`). Layer 2 (`--verify`, ~150–200 gpt-4o-mini calls/week ≈ a few cents, logged to `usage_log` as operation `verify`) reads each transcript and checks the content actually supports the claimed show — catching reruns, pledge drives, substitute programming, and dead air the log alone can't reveal. Its prompt is the `verify_prompt` setting (default `DEFAULT_VERIFY_PROMPT` in `lib/settings.ts`, per-station override like every other AI prompt), and the pass honors the spend limits (skipped/aborted when over budget). Output: terminal summary + markdown/JSON reports in `reports/` (gitignored); each run leaves an `audit_log` row (`verify.week.complete`). It's a review aid, not pass/fail — preemptions are legitimate; a human decides. The engine is pure and unit-tested (`lib/verify-week.ts`); the driver (`scripts/verify-week.ts`) needs the worker env and is read-only apart from usage + audit logging.

**Fund-drive pitch analytics** (`npm run pitch-report -- --station <slug> [--date D | --start S --end E] [--show <key>]`) answers "which shows and hours pitched during the drive, how many times, and for how long?" — the development-side counterpart to the FCC reporting. The unit is a **pitch segment**, not a keyword hit (forty hits in an hour could be one 22-minute break or four 30-second spots), and it is measured off the timed cue surface (`transcript_cues`, migration 022) so duration comes from the cue clock without touching audio. Detection is deliberate about false positives: each cue is scored against a **tiered lexicon** — *strong* (an explicit ask: pledge, fund drive, "contribute online at kpfk.org", premiums, sustainers, matching gifts, the Spanish "llame al …/donación"), *medium* (donation-specific but occasionally ordinary: donation, membership, "call 818-…", the station URL, gift certificates), *weak* (money/support language) — because on a real KPFK drive day "$/dollars" fires 324 times and most of them are news copy ("$1.8 billion fund"), and the hourly legal ID names `kpfk.org` year-round. A cue is marked if it carries a strong term, or if its ±45s neighbourhood scores ≥4 **and** contains a medium-or-better term; marked cues within 45s merge into one segment (hosts chat between asks); sub-20s dust is dropped unless strong-anchored or densely medium (a 15s produced spot survives, a talk show reading its own call-in line once does not). Two further gates keep the count honest: the **intent veto** (every term is `kind: 'ask' | 'phone' | 'web'`, and a segment with no non-weak *ask* term is discarded — that's what separates a pitch from a station promo or the legal ID reading the same URL), and the **timeline guard** (an episode whose transcript arrived as one giant caption has no usable clock, so it yields no segments and is reported as unmeasured rather than as one 2-hour pitch). **Detection confidence and ask quality are separate axes**: `tier` says how sure we are a pitch happened; the per-segment `AskProfile` says what it asked for — channels named (`complete` = phone **and** web, `partial` = one, `none` = neither), dollar amounts (station goal/shortfall figures are subtracted, so "our goal is $200,000" never reads as an ask), sustainer, premium, matching, deadline. Roll-ups: per show (grouped by `show_group`), per **clock hour** (segments straddling `:00` are split at the boundary, counted once in the hour they began), per day, and **per asker** (`show_group` + `episode_log.host`) — pitches, pitch time, mean/longest break, channel completeness, and the level they drive to (`usualAmount`: the repeated figure, else the median — never just the biggest number they said). Output: terminal summary + markdown, a **segments CSV** (one row per pitch: clock time, length, channels, amounts, flags, excerpt — the shape to sit next to the donation log), an **askers CSV** (the scorecard) and an hours CSV in `reports/` (gitignored); each run leaves an `audit_log` row (`pitch.report.complete`). Station-specific asks (a campaign URL, a shortcode, the pledge line itself) can be added per station via the `pitch_terms` setting without a deploy. **It is a floor, not a measurement**: this free layer counts *explicit* asks and under-reports soft pitching that never names one, and an airing with no transcript counts as zero (reported separately). An AI pass over candidate regions is the intended second layer. Engine is pure and unit-tested (`lib/pitch-report.ts`); the driver (`scripts/pitch-report.ts`) needs the worker env, costs nothing to run, and writes only its audit row.
